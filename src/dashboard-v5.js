(() => {
  ...
  const SHEET_PURPOSES = {
    ...
    Demand_Pattern_Design: "Declared and observed V5 ADI/CV2 fixture behavior",
  ...
  function renderForecast() {
    ...
    el("forecast-interval-legend").textContent = bundle.has95 ? "80% / 95%" : bundle.has80 ? "80% q10-q90" : "interval unavailable";
    const notes = [];
    ...
    if (bundle.selected === "TiRex2") notes.push("TiRex2 quantiles use non-negative clipping and monotone rearrangement; q50 and marginal q10-q90 are shown, with no native 95% interval claimed.");
  ...
  function renderAdjustments(sku, bundle) {
    ...
    const baselineAdu = num(position?.ADU);
    const dlt = num(position?.DLT_Days);
    el("adjustment-interval-label").textContent = bundle.has95 ? "95% interval" : bundle.has80 ? "80% q10-q90" : "Interval";
  ...
  function renderSegmentation() {
    ...
    const patternOrder = { Lumpy: 0, Intermittent: 1, Erratic: 2, Smooth: 3 };
    const tableRows = state.segmentationMode === "behavior"
      ? [...rows].sort((a, b) => (patternOrder[a.Demand_Pattern] ?? 9) - (patternOrder[b.Demand_Pattern] ?? 9) || num(b.Annual_Usage_Value) - num(a.Annual_Usage_Value))
      : rows;
    const patternCounts = rows.reduce((counts, row) => {
      const key = row.Demand_Pattern || "Unclassified";
      counts[key] = (counts[key] || 0) + 1;
      return counts;
    }, {});
    el("segmentation-evidence").textContent = state.segmentationMode === "behavior"
      ? `36 complete months | Smooth ${patternCounts.Smooth || 0} | Erratic ${patternCounts.Erratic || 0} | Intermittent ${patternCounts.Intermittent || 0} | Lumpy ${patternCounts.Lumpy || 0}. Table ranks behavior exceptions first, then usage value.`
      : "ABC uses annual usage value; XYZ uses the complete-history demand coefficient of variation. Classes are communication aids, not standalone policies.";
    el("segmentation-body").innerHTML = tableRows.length ? tableRows.slice(0, 24).map((row) => `<tr><td><strong>${esc(row.Part_ID)}</strong><small>${esc(row.product.Product_Group || "")}</small></td><td class="number">${money(row.Annual_Usage_Value)}</td><td><span class="abc-class class-${String(row.Usage_Value_ABC || "c").toLowerCase()}">${esc(row.ABC_XYZ || "-")}</span></td><td>${esc(row.Demand_Pattern || "-")}</td><td>${esc(row.Criticality || row.product.Operational_Criticality || "-")}</td><td class="number">${fmt(row.product.Qualified_Source_Count)}</td></tr>`).join("") : '<tr><td colspan="6" class="empty-state">No item segmentation matches the active commercial filters.</td></tr>';
    if (!rows.length) {
    ...
    const thresholds = state.segmentationMode === "behavior"
      ? `<line class="portfolio-boundary" x1="${x(1.32)}" x2="${x(1.32)}" y1="${margin.top}" y2="${margin.top + plotH}"></line><line class="portfolio-boundary" x1="${margin.left}" x2="${margin.left + plotW}" y1="${y(0.49)}" y2="${y(0.49)}"></line><text class="quadrant-label" x="${x((xDomain[0] + 1.32) / 2)}" y="${y(0.20)}">Smooth</text><text class="quadrant-label" x="${x((xDomain[0] + 1.32) / 2)}" y="${y((0.49 + yMax) / 2)}">Erratic</text><text class="quadrant-label" x="${x((1.32 + xDomain[1]) / 2)}" y="${y(0.20)}">Intermittent</text><text class="quadrant-label" x="${x((1.32 + xDomain[1]) / 2)}" y="${y((0.49 + yMax) / 2)}">Lumpy</text><text class="svg-label" x="${x(1.32) + 5}" y="${margin.top + plotH - 5}">ADI 1.32</text><text class="svg-label" x="${margin.left + 5}" y="${y(0.49) - 5}">CV2 0.49</text>`
  ...
  function exportPlan() {
    downloadCsv("demand-genie-v5-recommendations.csv", filteredPositions().map((row) => ({
      SKU: row.SKU,
  ...
  function exportForecast() {
    ...
    downloadCsv(`demand-genie-v5-forecast-${state.selectedSku}.csv`, bundle.forecast.map((row) => ({
  ...
  function exportSpend() {
    ...
    downloadCsv("demand-genie-v5-spend-analysis.csv", [...categories, ...suppliers]);
  }
  ...
  function bindEvents() {
    ...
    el("export-audit").addEventListener("click", () => downloadCsv("demand-genie-v5-planner-action-log.csv", state.actionLog));
