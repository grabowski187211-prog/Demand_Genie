(() => {
  ...
  function renderForecast() {
    ...
    el("engine-state").textContent = `Forecast: ${bundle.source === "fpp3" ? state.data.forecastEngine : "local browser fallback"} | Spend: ${state.data.spendEngine || "local auditable analysis"}`;
  ...
  function renderPareto(rows) {
    ...
    const width = 900;
    const rowHeight = 30;
    const height = 48 + visible.length * rowHeight;
    const margin = { top: 16, right: 52, bottom: 26, left: 185 };
    const plotW = width - margin.left - margin.right;
    ...
    svg.setAttribute("viewBox", `0 0 ${width} ${height}`);
    svg.innerHTML = `${[0, 0.5, 1].map((value) => `<line class="svg-grid" x1="${margin.left + value * plotW}" x2="${margin.left + value * plotW}" y1="${margin.top - 7}" y2="${height - margin.bottom}"></line><text class="svg-label" x="${margin.left + value * plotW}" y="${height - 8}" text-anchor="middle">${money(maxValue * value)}</text>`).join("")}
      ...
      <line class="pareto-threshold" x1="${margin.left + plotW * 0.8}" x2="${margin.left + plotW * 0.8}" y1="${margin.top - 7}" y2="${height - margin.bottom}"></line>
  ...
  function renderData() {
    ...
    el("data-headline").textContent = requiredMissing.length ? `Workbook is missing ${requiredMissing.join(" and ")}.` : "Workbook is complete for planning and spend analysis.";
    const failedChecks = sheet("Spend_Data_Quality").filter((row) => row.Status === "FAIL").length;
    el("data-subline").textContent = failedChecks ? `${failedChecks} spend quality gates failed; financial interpretation is blocked.` : "Demand, planning, procurement, risk evidence, and the control total are loaded as separate facts.";
  ...
  function exportPlan() {
    downloadCsv("demand-genie-v3-recommendations.csv", filteredPositions().map((row) => ({
      SKU: row.SKU,
  ...
  function exportForecast() {
    ...
    downloadCsv(`demand-genie-v3-forecast-${state.selectedSku}.csv`, bundle.forecast.map((row) => ({
  ...
  function exportSpend() {
    ...
    downloadCsv("demand-genie-v3-spend-analysis.csv", [...categories, ...suppliers]);
  }
  ...
  function browserItemSegmentation(rows) {
    ...
    const output = [...groups.entries()].map(([key, members]) => {
      ...
      const values = members.map((row) => Math.max(0, num(row.Demand_Units)));
    ...
    }).filter((row) => row.Annual_Usage_Value > 0).sort((a, b) => b.Annual_Usage_Value - a.Annual_Usage_Value);
    const total = output.reduce((sum, row) => sum + row.Annual_Usage_Value, 0);
    let cumulative = 0;
  ...
  function bindEvents() {
    ...
    el("export-audit").addEventListener("click", () => downloadCsv("demand-genie-v3-planner-action-log.csv", state.actionLog));
