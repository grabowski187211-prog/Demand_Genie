"""Numerical and responsive browser tests for Demand Genie Dashboard V5."""
...
URL = sys.argv[1] if len(sys.argv) > 1 else "http://127.0.0.1:8001/dashboard-v5.html"
WORKBOOK = ROOT / "data" / "Demand_Genie_Synthetic_Portfolio_v5.xlsx"
...
def verify_core(page: Page, viewport_name: str, expected: dict[str, object]) -> dict[str, object]:
    ...
    assert page.locator("#forecast-interval-legend").inner_text() == "80% q10-q90"
    assert page.locator("#adjustment-interval-label").inner_text().lower() == "80% q10-q90"
    ...
    assert download_info.value.suggested_filename == f"demand-genie-v5-forecast-{expected['sku']}.csv"
    ...
    assert spend_download.value.suggested_filename == "demand-genie-v5-spend-analysis.csv"
    ...
    for pattern, expected_count in (("smooth", 30), ("erratic", 18), ("intermittent", 17), ("lumpy", 15)):
        assert page.locator(f"#segmentation-chart .segmentation-point.{pattern}").count() == expected_count
    assert "Smooth 30" in page.locator("#segmentation-evidence").inner_text()
    assert page.locator("#segmentation-body tr").first.locator("td").nth(3).inner_text() == "Lumpy"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with sync_playwright() as playwright:
            ...
            for name, viewport in (
                ...
                page.get_by_role("tab", name="Portfolio").click()
                screenshot = f"/tmp/demand-genie-v5-{name}-segmentation.png"
                page.screenshot(path=screenshot, full_page=True)
                screenshots.append(screenshot)
                page.get_by_role("tab", name="Forecast").click()
                page.locator('#forecast-mode-control button[data-forecast-mode="decomposition"]').click()
                screenshot = f"/tmp/demand-genie-v5-{name}-decomposition.png"
                page.screenshot(path=screenshot, full_page=True)
                screenshots.append(screenshot)
                page.get_by_role("tab", name="Data").click()
                screenshot = f"/tmp/demand-genie-v5-{name}-data.png"
                page.screenshot(path=screenshot, full_page=True)
                screenshots.append(screenshot)
                page.close()
